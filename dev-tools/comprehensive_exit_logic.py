...
class ComprehensiveExitChecker:
    ...
    def _check_breakeven(self, profit_ticks, r_multiple, current_price, contracts, triggered):
        ...
        if not self.trade.get('breakeven_active', False):
            bars_in_trade_current = self.trade.get('bars_in_trade', 0)
            adjusted_threshold = threshold_ticks * breakeven_mult
            
            if profit_ticks >= adjusted_threshold and bars_in_trade_current >= min_duration:
    ...
    def _check_trailing_stops(self, current_bar, current_price, side, profit_ticks, triggered):
        ...
        initial_risk = self.trade.get('initial_risk_ticks', 1)
        r_mult = profit_ticks / initial_risk if initial_risk > 0 else 0
    ...
    def _check_time_based_exits(self, current_bar, bars_in_trade, profit_ticks, 
        ...
        # 5. Forced exit time (flatten mode before maintenance)
        current_time = current_bar['timestamp'].time()
        forced_hour = 21  # 21:45 UTC - flatten mode starts (15 min before maintenance)
        forced_minute = 45
        triggered.append('forced_exit_time_hour')
        
        if current_time.hour > forced_hour or (current_time.hour == forced_hour and current_time.minute >= forced_minute):
            return {
                'should_exit': True,
    ...
    def _check_adverse_conditions(self, current_bar, all_bars, bar_index, profit_ticks, 
        ...
        MIN_R_FOR_PROTECTION = self.exit_params.get('profit_protection_min_r', self.exit_params.get('profit_protection_min_r', 2.5))
